...
void mpi_xcrypt(MPI_attrs *attrs) {
  for (size_t i = 0; i < attrs->size_slice; i+=AES_BLOCKLEN) {
    ...
    for(int j = 0; j < AES_BLOCKLEN; j++) {
      buf[attrs->start_position + i + j] = buf[attrs->start_position + i + j] ^ attrs->vector[j];
    }
...
int main(void) {
  ...
  //uint8_t *buffer_slice = (uint8_t *)malloc(sizeof(uint8_t) * size / N_PROCESS);
  ...
  attrs.start_position = slice * world_rank;
